...
<div class="container">
    ...
    <form name = "order_form" class="form-horizontal" action="<?=base_url()?>order" method="post">
        ...
        <div class="form-group" id="form_group_id">
            <label class="control-label <?php if (form_error('id')!=='' ) echo 'd-none'?>" for="id">ID товара:</label>
            <?php echo form_error('id', "<label class='control-label form-error' for='id'>", "</label>");?>
            <div id="form_id">
                <input type="text" class="form-control <?php if (form_error('id')!=='') echo 'form-error-text'?>" id="id" name="id" value="<?php if (set_value('id')!=='') echo set_value('id'); else echo $id; ?>"
            </div>
        </div>

        <div class="form-group" id="form_group_product_name">
            <label class="control-label <?php if (form_error('product_name')!=='') echo 'd-none'?>" for="product_name">Наименование товара:</label>
            <?php echo form_error('id', "<label class='control-label form-error' for='product_name'>", "</label>")?>
            <div id="form_product_name">
                <input type="text" class="form-control <?php if (form_error('product_name')!=='') echo 'form-error-text'?>" id="product_name" name="product_name" value="<?php echo set_value('product_name'); ?>">
            </div>
        </div>
        ...
        <div class="form-group" id="form_group_comment">
            <label class="control-label <?php if (form_error('comment')!=='') echo 'd-none'?>" for="comment">Пожелания и комментарии:</label>
    ...
    <script>
        ...
        form.onsubmit = function(){
            ...
            if ((id.value == '' || id.value == null) && (product_name.value == '' || product_name.value == null)) {
                ...
                setTimeout(function(){element1.removeChild(alert1)}, time);
                setTimeout(function(){_element1.removeChild(_alert1)}, time);
            }
